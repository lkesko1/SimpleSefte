...
function validateFormRezervacija() {
	...
	{
		if (x[i].value == "" || x[i].value==null || (forma['date'] == x[i] && forma['date'].value < datum) || ((!telefonRegEx.test(forma['tel'].value)) && x[i]==forma['tel']) || ((!imeRegex.test(forma['ime'].value)) && x[i]==forma['ime'])) {
			...
			else if (x[i]==forma['tel'] && forma['date'].value < datum){
